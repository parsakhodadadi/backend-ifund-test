<ul class="metismenu" id="menu">
    <li>
        <a href="javascript:;" class="has-arrow">
            <div class="parent-icon"><i class='bx bx-home'></i>
            </div>
            <div class="menu-title">خانه</div>
        </a>
        <ul>
            <li> <a href="index.html"><i class="bx bx-left-arrow-alt"></i>داشبورد</a>
            </li>
        </ul>
    </li>
    <li>
        <a href="javascript:;" class="has-arrow">
            <div class="parent-icon"><i class='bx bx-spa'></i>
            </div>
            <div class="menu-title">
                <?php echo e(__('category.categories')); ?>

            </div>
        </a>
        <ul>
            <li> <a href="<?php echo e(route('/admin/category')); ?>"><i class="bx bx-left-arrow-alt"></i><?php echo e(__('category.create-category')); ?></a>
            </li>
            <li> <a href="<?php echo e(route('/admin/category/list')); ?>"><i class="bx bx-left-arrow-alt"></i><?php echo e(__('category.settings')); ?></a>
            </li>
        </ul>
    </li>
    <li>
        <a href="javascript:;" class="has-arrow">
            <div class="parent-icon"><i class='bx bx-cart-alt'></i>
            </div>
            <div class="menu-title">مدیریت محصولات</div>
        </a>
        <ul>
            <li> <a href="<?php echo e(route('/Others/Themes/Backend/main/vertical/')); ?>ecommerce-products.html"><i class="bx bx-left-arrow-alt"></i>ایجاد محصول جدید</a>
            </li>
            <li> <a href="<?php echo e(route('/Others/Themes/Backend/main/vertical/')); ?>ecommerce-products-details.html"><i class="bx bx-left-arrow-alt"></i>بروز رسانی محصولات</a>
            </li>
            <li> <a href="<?php echo e(route('/Others/Themes/Backend/main/vertical/')); ?>ecommerce-orders.html"><i class="bx bx-left-arrow-alt"></i>سفارشات</a>
            </li>
        </ul>
    </li>
    <li>
        <a class="has-arrow" href="javascript:;">
            <div class="parent-icon"><i class='bx bx-gift'></i>
            </div>
            <div class="menu-title">طرح بندی</div>
        </a>
    </li>
    <li>
        <a href="<?php echo e(route('/logout')); ?>">
            <div class="parent-icon"><i class='bx bx-user-pin'></i>
            </div>
            <div class="menu-title">خروج</div>
        </a>
    </li>
</ul><?php /**PATH /Applications/MAMP/htdocs/ParsaFramework/views/Backend/main/layout/navigation.blade.php ENDPATH**/ ?>