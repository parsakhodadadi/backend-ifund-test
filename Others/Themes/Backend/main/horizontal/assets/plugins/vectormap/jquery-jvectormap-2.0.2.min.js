! function (t) {
    ...
    t.fn.vectorMap = function (t) {
        ...
        if ("addMap" === t) jvm.Map.maps[arguments[1]] = arguments[2];
            ...
            if (("set" === t || "get" === t) && e[t][arguments[1]]) return a[t + (arguments[1].charAt(0).toUpperCase() + arguments[1].substr(1))].apply(a, Array.prototype.slice.call(arguments, 2));
...
function (t) {
    ...
    var e, a, s = ["wheel", "mousewheel", "DOMMouseScroll", "MozMousePixelScroll"],
        i = "onwheel" in document || document.documentMode >= 9 ? ["wheel"] : ["mousewheel", "DomMouseScroll", "MozMousePixelScroll"],
        n = Array.prototype.slice;
    if (t.event.fixHooks)
        for (var r = s.length; r;) t.event.fixHooks[s[--r]] = t.event.mouseHooks;
    var h = t.event.special.mousewheel = {
        ...
        teardown: function () {
            if (this.removeEventListener)
                for (var t = i.length; t;) this.removeEventListener(i[--t], o, !1);
    ...
    function o(s) {
        var i, r = s || window.event,
            h = n.call(arguments, 1),
            o = 0,
            c = 0,
            p = 0;
        if ((s = t.event.fix(r)).type = "mousewheel", "detail" in r && (p = -1 * r.detail), "wheelDelta" in r && (p = r.wheelDelta), "wheelDeltaY" in r && (p = r.wheelDeltaY), "wheelDeltaX" in r && (c = -1 * r.wheelDeltaX), "axis" in r && r.axis === r.HORIZONTAL_AXIS && (c = -1 * p, p = 0), o = 0 === p ? c : p, "deltaY" in r && (o = p = -1 * r.deltaY), "deltaX" in r && (c = r.deltaX, 0 === p && (o = -1 * c)), 0 !== p || 0 !== c) {
            if (1 === r.deltaMode) {
            ...
            return i = Math.max(Math.abs(p), Math.abs(c)), (!a || i < a) && (a = i, m(r, i) && (a /= 40)), m(r, i) && (o /= 40, c /= 40, p /= 40), o = Math[o >= 1 ? "floor" : "ceil"](o / a), c = Math[c >= 1 ? "floor" : "ceil"](c / a), p = Math[p >= 1 ? "floor" : "ceil"](p / a), s.deltaX = c, s.deltaY = p, s.deltaFactor = a, s.deltaMode = 0, h.unshift(s, o, c, p), e && clearTimeout(e), e = setTimeout(l, 200), (t.event.dispatch || t.event.handle).apply(this, h)
        }
    }
...
jvm.$ = jQuery, Array.prototype.indexOf || (Array.prototype.indexOf = function (t, e) {
    ...
    for (a = Math.max(n >= 0 ? n : i - Math.abs(n), 0); a < i;) {
    ...
    var e, a, s = 0,
        i = 0;
    return (t = t.replace(/(-?\d+)e(-?\d+)/g, "0")).replace(/([MmLlHhVvCcSs])\s*((?:-?\d*(?:\.\d+)?\s*,?\s*)+)/g, function (t, n, r, h) {
        (r = r.replace(/(\d)-/g, "$1,-").replace(/^\s+/g, "").replace(/\s+$/g, "").replace(/\s+/g, ",").split(","))[0] || r.shift();
        for (var o = 0, l = r.length; o < l; o++) r[o] = Math.round(100 * r[o]);
        switch (n) {
            case "m":
    ...
    getValue: function (t) {
        ...
        return t = n == this.scale.length - 1 ? this.vectorToNum(this.scale[n]) : this.vectorToNum(this.vectorAdd(this.scale[n], this.vectorMult(this.vectorSubtract(this.scale[n + 1], this.scale[n]), t / s[n])))
    },
    ...
    var s;
    (t = t || {}).attribute = t.attribute || "fill", this.elements = e, this.params = t, this.map = a, t.attributes && this.setAttributes(t.attributes), jvm.$.isArray(t.scale) ? (s = "fill" === t.attribute || "stroke" === t.attribute ? jvm.ColorScale : jvm.NumericScale, this.scale = new s(t.scale, t.normalizeFunction, t.min, t.max)) : t.scale ? this.scale = new jvm.OrdinalScale(t.scale) : this.scale = new jvm.SimpleScale(t.scale), this.values = t.values || {}, this.setValues(this.values), this.params.legend && (this.legend = new jvm.Legend($.extend({
        map: this.map,
        series: this
    }, this.params.legend)))
    ...
    setValues: function (t) {
        ...
        if (this.scale instanceof jvm.OrdinalScale || this.scale instanceof jvm.SimpleScale)
            for (a in t) t[a] ? n[a] = this.scale.getValue(t[a]) : n[a] = this.elements[a].element.style.initial[this.params.attribute];
        else {
            if (void 0 === this.params.min || void 0 === this.params.max)
                for (a in t)(e = parseFloat(t[a])) > s && (s = e), e < i && (i = e);
    ...
    aea: function (t, e, a) {
        var s = a * this.radDeg,
            ...
            r = t * this.radDeg,
            h = e * this.radDeg,
            o = (Math.sin(i) + Math.sin(n)) / 2,
            l = Math.cos(i) * Math.cos(i) + 2 * o * Math.sin(i),
            m = o * (h - s),
            c = Math.sqrt(l - 2 * o * Math.sin(r)) / o,
            p = Math.sqrt(l - 2 * o * Math.sin(0)) / o;
        return {
    ...
    aea_inv: function (t, e, a) {
        var s = t / this.radius,
            ...
            r = 29.5 * this.radDeg,
            h = 45.5 * this.radDeg,
            o = (Math.sin(r) + Math.sin(h)) / 2,
            l = Math.cos(r) * Math.cos(r) + 2 * o * Math.sin(r),
            m = Math.sqrt(l - 2 * o * Math.sin(0)) / o,
            c = Math.sqrt(s * s + (m - i) * (m - i)),
            p = Math.atan(s / (m - i));
        return {
            lat: Math.asin((l - c * c * o * o) / (2 * o)) * this.degRad,
            lng: (n + p / o) * this.degRad
        }
    },
    lcc: function (t, e, a) {
        var s = a * this.radDeg,
            ...
            r = 45 * this.radDeg,
            h = t * this.radDeg,
            o = Math.log(Math.cos(n) * (1 / Math.cos(r))) / Math.log(Math.tan(Math.PI / 4 + r / 2) * (1 / Math.tan(Math.PI / 4 + n / 2))),
            l = Math.cos(n) * Math.pow(Math.tan(Math.PI / 4 + n / 2), o) / o,
            m = l * Math.pow(1 / Math.tan(Math.PI / 4 + h / 2), o),
            c = l * Math.pow(1 / Math.tan(Math.PI / 4 + 0), o);
        return {
            x: m * Math.sin(o * (i - s)) * this.radius,
            y: -(c - m * Math.cos(o * (i - s))) * this.radius
        }
    },
    lcc_inv: function (t, e, a) {
        var s = t / this.radius,
            ...
            l = Math.cos(r) * Math.pow(Math.tan(Math.PI / 4 + r / 2), o) / o,
            m = l * Math.pow(1 / Math.tan(Math.PI / 4 + 0), o),
            c = this.sgn(o) * Math.sqrt(s * s + (m - i) * (m - i)),
            p = Math.atan(s / (m - i));
        return {
            lat: (2 * Math.atan(Math.pow(l / c, 1 / o)) - Math.PI / 2) * this.degRad,
            lng: (n + p / o) * this.degRad
        }
    }
}, jvm.MapObject = function (t) {}, jvm.MapObject.prototype.getLabelText = function (t) {
    ...
    bindContainerEvents: function () {
        ...
        this.params.panOnDrag && (this.container.mousemove(function (i) {
            ...
            a = !1
        }, jvm.$("body").mouseup(this.onContainerMouseUp)), this.params.zoomOnScroll && this.container.mousewheel(function (t, e, a, i) {
            var n = jvm.$(s.container).offset(),
                r = t.pageX - n.left,
                h = t.pageY - n.top,
                o = Math.pow(1 + s.params.zoomOnScrollSpeed / 1e3, t.deltaFactor * t.deltaY);
            s.tip.hide(), s.setScale(s.scale * o, r, h), t.preventDefault()
        })
    },
    ...
    bindElementEvents: function () {
        ...
        this.container.mousemove(function () {
            ...
            var a = -1 === (jvm.$(this).attr("class").baseVal || jvm.$(this).attr("class")).indexOf("jvectormap-region") ? "marker" : "region",
                s = "region" == a ? jvm.$(this).attr("data-code") : jvm.$(this).attr("data-index"),
                i = "region" == a ? e.regions[s].element : e.markers[s].element,
                n = "region" == a ? e.mapData.paths[s].name : e.markers[s].config.name || "",
                r = jvm.$.Event(a + "TipShow.jvectormap"),
                h = jvm.$.Event(a + "Over.jvectormap");
            "mouseover" == t.type ? (e.container.trigger(h, [s]), h.isDefaultPrevented() || i.setHovered(!0), e.tip.text(n), e.container.trigger(r, [e.tip, s]), r.isDefaultPrevented() || (e.tip.show(), e.tipWidth = e.tip.width(), e.tipHeight = e.tip.height())) : (i.setHovered(!1), e.tip.hide(), e.container.trigger(a + "Out.jvectormap", [s]))
        }), this.container.delegate("[class~='jvectormap-element']", "mousedown", function () {
            t = !1
        }), this.container.delegate("[class~='jvectormap-element']", "mouseup", function () {
            var a = -1 === (jvm.$(this).attr("class").baseVal ? jvm.$(this).attr("class").baseVal : jvm.$(this).attr("class")).indexOf("jvectormap-region") ? "marker" : "region",
                s = "region" == a ? jvm.$(this).attr("data-code") : jvm.$(this).attr("data-index"),
                i = jvm.$.Event(a + "Click.jvectormap"),
                n = "region" == a ? e.regions[s].element : e.markers[s].element;
            t || (e.container.trigger(i, [s]), ("region" === a && e.params.regionsSelectable || "marker" === a && e.params.markersSelectable) && (i.isDefaultPrevented() || (e.params[a + "sSelectableOne"] && e.clearSelected(a + "s"), n.setSelected(!n.isSelected))))
        })
    },
    ...
    createTip: function () {
        ...
        this.tip = jvm.$("<div/>").addClass("jvectormap-tip").appendTo(jvm.$("body")), this.container.mousemove(function (e) {
            ...
            a < 5 && (a = e.pageX + 15), s < 5 && (s = e.pageY + 15), t.tip.css({
                left: a,
                top: s
            })
        })
    ...
    setScale: function (t, e, a, s, i) {
        var n, r, h, o, l, m, c, p, d, v = jvm.$.Event("zoom.jvectormap"),
            u = this,
            g = 0,
    ...
    pointToLatLng: function (t, e) {
        ...
        for (a = 0; a < l.length; a++)
            if (i = (s = l[a]).bbox, n = t - (this.transX * this.scale + s.left * this.scale), r = e - (this.transY * this.scale + s.top * this.scale), n = n / (s.width * this.scale) * (i[1].x - i[0].x) + i[0].x, r = r / (s.height * this.scale) * (i[1].y - i[0].y) + i[0].y, n > i[0].x && n < i[1].x && r > i[0].y && r < i[1].y) return jvm.Proj[h.type + "_inv"](n, -r, o);
        return !1
    },
