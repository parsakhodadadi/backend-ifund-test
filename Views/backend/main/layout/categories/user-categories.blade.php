                    ...
                    <table id="example2" class="table table-striped table-bordered">
                        ...
                        @foreach($categories as $category)
                            <tr>
                                ...
                                @else
                                    <td>
                                        <h6 class="">{{ $lang['disapproved'] }}</h6>
                                    </td>
                                @endif
                                <td>
                                    <a class="btn btn-primary"
                                       href="{{ route('/panel/my-categories/edit/') . $category->id }}">{{ $lang['edit'] }}</a>
                                    <a class="btn btn-danger"
                                       href="{{ route('/panel/my-categories/delete/') . $category->id }}">{{ $lang['delete'] }}</a>
                                </td>
                                <td>
                                    @if($category->status == 'approved')
                                        <a class="btn btn-primary"
                                           href="{{ route('/panel/my-categories/') . $category->id . '/add-subject' }}">+</a>
                                        <a class="btn btn-dark"
                                           href="{{ route('/panel/my-categories/') . $category->id . '/subjects' }}">{{ $lang['list'] }}</a>
                                    @endif
                                </td>
                            </tr>
                        @endforeach
